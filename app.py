...
def check_winner(b, size):
    ...
    if all(b[i][i] == b[0][0] and b[0][0] != ' ' for i in range(size)):
        return b[0][0]
    # Chéo phụ
    if all(b[i][size - 1 - i] == b[0][size - 1] and b[0][size - 1] != ' ' for i in range(size)):
        return b[0][size - 1]
    return None
...
def minimax(board, depth, is_maximizing, alpha, beta, size, max_depth):
    ...
    if is_full(board, size) or depth == max_depth: return 0
    ...
    if is_maximizing:
        ...
        for r in range(size):
            for c in range(size):
                if board[r][c] == ' ':
                    board[r][c] = 'O'
                    eval = minimax(board, depth + 1, False, alpha, beta, size, max_depth)
                    board[r][c] = ' '
    ...
    for r in range(size):
        for c in range(size):
            if st.session_state.board[r][c] == ' ':
                ...
                score = minimax(st.session_state.board, 0, False, -float('inf'), float('inf'), size, max_depth)
...
size = st.session_state.size
st.write(f"Trạng thái: **{'Lượt của bạn (X)' if st.session_state.turn == 'X' else 'AI đang đi...'}**")
...
for r in range(size):
    ...
    for c in range(size):
        ...
        if cols[c].button(label, key=f"btn_{r}_{c}"):
            if val == ' ' and st.session_state.turn == 'X' and not st.session_state.winner:
